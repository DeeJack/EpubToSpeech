...
tts_model = tts_namespace.model(
    "TTS",
    {
        ...
        "service": fields.String(
            required=True, description="The TTS service to be used", enum=['azure', 'local', 'elevenlabs', 'openai']
        ),
    },
)

@tts_namespace.route("/")
@tts_namespace.doc(responses={200: 'OK', 400: 'Invalid Argument', 500: 'Mapping Key Error'}, description='TTS API endpoint', params={'book_id': 'The book ID', 'chapter': 'The chapter to be processed', 'service': 'The TTS service to be used'})
class TTS(Resource):
    ...
    def post(self):
        ...
        response = requests.post(f"{url}/storage/store-file", files={
            'file': audio_bytes
        }, data={
            'filename': f'{unique_filename}.wav'
        })
        if response.status_code != 200:
            return abort(response.status_code, response)
        
        # Add chapter to database
        response = requests.post(f"{url}/database/add-chapter", json={
            'book_id': book_id,
            'number': chapter,
