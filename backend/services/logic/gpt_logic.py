...
text_model = gptlogic_namespace.model(
    "Text",
    {
        "text": fields.String(required=True, description="The text to be processed"),
    },
)
...
class Summarize(Resource):
    ...
    def post(self):
        ...
        response = requests.post(
            f'{current_app.config["API_URL"]}/openai/text-generation',
            json={"pre_prompt": "Summarize the text given", "prompt": text},
        )
...
@gptlogic_namespace.doc(
    ...
    params={"text": "The text to be translated"},
)
class Translate(Resource):
    ...
    def post(self):
        ...
        response = requests.post(
            f'{current_app.config["API_URL"]}/openai/text-generation',
            json={"pre_prompt": "Translate the text given in english", "prompt": text},
        )
        if response.status_code != 200:
            return abort(response.status_code, response.json()["message"])
        return response.json()
...
@gptlogic_namespace.doc(
    ...
    params={"prompt": "The prompt for GPT", "text": "The text to be processed"},
)
class Generate(Resource):
    ...
    def post(self):
        ...
        response = requests.post(
            f'{current_app.config["API_URL"]}/openai/text-generation',
            json={"prompt": text, "pre_prompt": prompt},
        )
        if response.status_code != 200:
            return abort(response.status_code, response.json()["message"])
        return response.json()
...
@gptlogic_namespace.doc(
    ...
    params={"text": "The prompt to be processed"},
)
class Image(Resource):
    ...
    def post(self):
        ...
        response = requests.post(
            f'{current_app.config["API_URL"]}/openai/image-generation',
            json={"text": text},
        )
        if response.status_code != 200:
            return abort(response.status_code, response.json()["message"])
